...
@section('content')
    <h1 class="text-xl font-bold mb-4">ویرایش دپارتمان</h1>
    ...
    <x-ui.card>
        <form method="POST" action="{{ route('admin.departments.update', $department) }}" class="space-y-4">
            ...
            <div>
                ...
                <x-ui.input type="text" name="name" value="{{ old('name', $department->name) }}" required />
                @error('name')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-sm mb-1">توضیحات</label>
                <x-ui.textarea name="description" rows="4">{{ old('description', $department->description) }}</x-ui.textarea>
            </div>
